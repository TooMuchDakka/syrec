...
protected:
    ...
    void createParserConfigFromOptions(const std::map<OptimizerOption, std::string>& loadedOptimizationOptions, std::optional<syrec::ReadProgramSettings>& parsedProgramSettings) const {
        ...
        std::string newSignalIdentsGeneratedByNoAdditionalLineSynthesisPrefix = "";
        ...
        for (const auto& [key, value] : loadedOptimizationOptions) {
            switch (key) {
                ...
                case NoAdditionalLineSynthesisNewSignalIdentsPrefix: {
                    wereNoAdditionalLineSynthesisConfigOperationsDefined = true;
                    newSignalIdentsGeneratedByNoAdditionalLineSynthesisPrefix = value;
                    ASSERT_TRUE(!newSignalIdentsGeneratedByNoAdditionalLineSynthesisPrefix.rfind("__")) << "Signal ident prefix for new signal generated by no additional line synthesis of assignments must start with two underscore characters, provided value was: " << value;
                }
            }
        }
