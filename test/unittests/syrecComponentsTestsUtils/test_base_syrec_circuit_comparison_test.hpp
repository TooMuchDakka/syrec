...
protected:
        ...
        for (const auto& [key, value] : loadedOptimizationOptions) {
            switch (key) {
                ...
                case NoAdditionalLineSynthesisNewSignalIdentsPrefix: {
                    wereNoAdditionalLineSynthesisConfigOperationsDefined = true;
                    ASSERT_TRUE(!value.rfind("__", 0)) << "Signal ident prefix for new signal generated by no additional line synthesis of assignments must start with two underscore characters, provided value was: " << value;
    ...
    [[nodiscard]] static T chooseValueForOptionWhereUserSuppliedOptionHasHighestPriority(OptimizerOption option, const std::map<OptimizerOption, std::string>& userSuppliedOptions, T& defaultValue, T& userSuppliedValue) {
        if (!userSuppliedOptions.count(option)) {
            return defaultValue;
        }
