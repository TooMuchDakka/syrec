...
namespace parser {
class SignalAccessRestrictionBaseTest: public ::testing::Test {
public:
    ...
    const static inline SignalAccessRestriction::SignalAccess restrictedBitRange       = SignalAccessRestriction::SignalAccess(restrictedBit, bitWidthOfReferenceSignal - 1);
...
TEST_P(SignalAccessRestrictionBaseTestForBitsOfSignal, CheckForBitsOfSignal) {
    ...
    const auto& restrictionToPerform = std::get<1>(testParamInfo);
    restrictionToPerform(restriction);

    const auto& validator = std::get<2>(testParamInfo);
    checkRestrictionsStatusForAllBitsGlobally(validator);
}
...
TEST_P(SignalAccessRestrictionBaseTestForBitRanges, ChecksForBitRangesOfSignal) {
    ...
    const auto& validator = std::get<2>(testParamInfo);
    checkRestrictionsStatusForAllBitRangesGlobally(validator);
}
...
TEST_P(SignalAccessRestrictionBaseTestForDimensions, ChecksForDimensionsOfSignal) {
    ...
    const auto& validator = std::get<2>(testParamInfo);
    checkRestrictionsStatusForDimensions(validator);
}
...
TEST_P(SignalAccessRestrictionBaseTestForValuesOfDimensions, ChecksForValuesOfDimensionsOfSignal) {
    ...
    const auto& validator = std::get<2>(testParamInfo);
    checkRestrictionsStatusForAllValuesPerDimensions(validator);
}
...
TEST_P(SignalAccessRestrictionBaseTestForBitsOfDimension, CheckForBitsOfDimensions) {
    ...
    const auto& validator = std::get<2>(testParamInfo);
    checkRestrictionsStatusForAllBitsOfAllDimensions(validator);
}
