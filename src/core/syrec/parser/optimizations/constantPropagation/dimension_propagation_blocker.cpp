...
void DimensionPropagationBlocker::blockSubstitutionForDimension(const std::optional<unsigned>& valueOfDimensionToBlock) {
    ...
    if (!valueOfDimensionToBlock.has_value()) {
        ...
        perValueOfDimensionBitRangeAccessRestrictionLookup.insert_or_assign(*valueOfDimensionToBlock, std::make_shared<BitRangeAccessRestriction>(BitRangeAccessRestriction(signalInformation.bitWidth, BitRangeAccessRestriction::BitRangeAccess(0, signalInformation.bitWidth - 1))));
    }
}
    ...
    if (isDimensionCompletelyBlocked) {
        ...
        for (unsigned int valueOfDimensionToBlockCompletely = 0; valueOfDimensionToBlockCompletely < numValuesForDimension; ++valueOfDimensionToBlockCompletely) {
            if (valueOfDimensionToBlockCompletely != blockedValueOfDimension) {
                perValueOfDimensionBitRangeAccessRestrictionLookup.insert_or_assign(valueOfDimensionToBlockCompletely, std::make_shared<BitRangeAccessRestriction>(signalInformation.bitWidth, BitRangeAccessRestriction::BitRangeAccess(0, signalInformation.bitWidth - 1)));
            }
        }
    }
    ...
    if (!bitRange.has_value()) {
        if (valueOfDimension.has_value()) {
            if (ignoreNotFullwidthBitRangeRestrictions) {
                const bool isDimensionCompletelyBlocked = dimensionBitRangeAccessRestriction.has_value() && (*dimensionBitRangeAccessRestriction)->isAccessCompletelyRestricted();
                const bool isAccessedValueOfDimensionCompletelyBlocked = perValueOfDimensionBitRangeAccessRestrictionLookup.count(*valueOfDimension) != 0
                    && perValueOfDimensionBitRangeAccessRestrictionLookup.at(*valueOfDimension)->isAccessCompletelyRestricted();
                return isDimensionCompletelyBlocked || isAccessedValueOfDimensionCompletelyBlocked;
            }
            return dimensionBitRangeAccessRestriction.has_value() || perValueOfDimensionBitRangeAccessRestrictionLookup.count(*valueOfDimension) != 0;
        }
...
bool DimensionPropagationBlocker::tryTrimAlreadyBlockedPartsFromRestriction(const std::optional<unsigned>& accessedValueOfDimension, const std::optional<BitRangeAccessRestriction::BitRangeAccess>& accessedBitRange, BitRangeAccessRestriction& bitRangeAccess) const {
    ...
    const bool hasRestrictionsForEveryValueOfDimension = perValueOfDimensionBitRangeAccessRestrictionLookup.size() == numValuesForDimension;
    ...
    const bool isAccessedBitRangeCoveredInAllValuesOfDimension = hasRestrictionsForEveryValueOfDimension && std::all_of(
            perValueOfDimensionBitRangeAccessRestrictionLookup.cbegin(),
            perValueOfDimensionBitRangeAccessRestrictionLookup.cend(),
