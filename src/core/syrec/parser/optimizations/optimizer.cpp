...
optimizations::Optimizer::OptimizationResult<syrec::Module> optimizations::Optimizer::handleModule(const syrec::Module& module, const optimizations::Optimizer::ExpectedMainModuleCallSignature& expectedMainModuleSignature) {
    ...
    if (assignmentWithoutAdditionalLineSynthesisOptimizer) {
        ...
        assignmentWithoutAdditionalLineSynthesisOptimizer->reloadGeneratableReplacementSignalName();    
    }
...
optimizations::Optimizer::OptimizationResult<syrec::Statement> optimizations::Optimizer::handleAssignmentStmt(const syrec::AssignStatement& assignmentStmt) {
    ...
    if (!doesAssignmentNotModifyAssignedToSignalValue) {
        if (const auto& activeSymbolTableScope = getActiveSymbolTableScope(); !skipCheckForSplitOfAssignmentInSubAssignments && activeSymbolTableScope.has_value() && assignmentWithoutAdditionalLineSynthesisOptimizer) {
...
bool optimizations::Optimizer::makeNewlyGeneratedSignalsAvailableInSymbolTableScope(const parser::SymbolTable::ptr& symbolTableScope, const parser::SymbolTable::ModuleIdentifier& identifierOfModuleToWhichSignalsWillBeAddedTo, const syrec::Variable::vec& newlyGeneratedSignalsUsedForReplacements) {
